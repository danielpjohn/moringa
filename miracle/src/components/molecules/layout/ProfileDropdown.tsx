...
import { useAuth } from "../../organisms/Login/AuthContext";
...
export default function ProfileDropdown({ isMobile = false }: { isMobile?: boolean }) {
    ...
    // Use a consistent light green for the profile avatar background
            ...
            >
                {isAuthenticated ? (
                    user?.avatar ? (
                        ...
                        <div className={`${isMobile ? "h-6 w-6" : "h-7 w-7"} rounded-full ${bgColor} flex items-center justify-center text-green-800 font-semibold hover:scale-105 transition-transform duration-200`}>
                            {userInitial}
                        </div>
                    )
                ) : (
                    <FaUserCircle
                        size={isMobile ? 24 : 28}
                        className="text-green-300 hover:scale-105 transition-transform duration-200"
                    />
                )}
