...
import { syncGuestCart, fetchCart, addToCart, saveGuestCart, getGuestCart } from "../Products/api.ts";
import type { CartItem } from "../Products/api.ts";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchUserData = useCallback(async (token: string) => {
    try {
      const response = await axios.get(`${API_ENDPOINTS.BASE_URL}/user/`, {
  ...
  const login = useCallback(async (userData: User, tokens: Tokens) => {
    ...
    try {
      ...
      saveGuestCart(updatedCart.map((item: CartItem) => ({
        id: item.id,
  ...
  const logout = useCallback(async () => {
    try {
      ...
      const currentCart = await fetchCart(localStorage.getItem('access_token') || '');
      if (currentCart.length > 0) {
        saveGuestCart(currentCart.map((item: CartItem) => ({
          id: item.id,
          product: item.product,
          quantity: item.quantity,
          addedAt: new Date().toISOString(),
